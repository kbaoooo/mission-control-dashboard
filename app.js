...
function runDashboard() {
    ...
    function buildFootprint(pos, sat) {
        ...
        const FOOTPRINT_SCALE = 0.72;
        const psi = Math.acos(EARTH_RADIUS_KM / (EARTH_RADIUS_KM + sat.altitudeKm)) * FOOTPRINT_SCALE;
        ...
        for (var bearingDeg = 0; bearingDeg <= 360; bearingDeg += 8) {
            ...
            const point = geoToMapPoint(radToDeg(lat2), radToDeg(lon2));
            if (point[0] < EDGE_MARGIN || point[0] > MAP_HEIGHT - EDGE_MARGIN) {
    ...
    const satObjects = satellites.map(function(sat, i) {
        ...
        const footprintGlow = L.polyline(buildFootprintSegments(initialPosition, sat), {
            color: clr,
            weight: 6,
            opacity: 0.2,
            lineCap: 'round',
    ...
    function setSatelliteVisible(sat, visible) {
        ...
        if (visible) {
            ...
            if (!map.hasLayer(sat.footprintGlow)) sat.footprintGlow.addTo(map);
        ...
        if (map.hasLayer(sat.footprintGlow)) map.removeLayer(sat.footprintGlow);
    ...
    function renderSatelliteToggleSidebar() {
        ...
        satObjects.forEach(function(sat) {
            ...
            row.innerHTML =
                '<input type="checkbox"' + (sat.visible ? ' checked' : '') + '>' +
                '<span class="sat-toggle-meta">' +
                '<span class="sat-toggle-name" style="color:' + sat.color + '">' + sat.name + '</span>' +
                '<span class="sat-toggle-details">' +
                '<span class="sat-toggle-line">Độ cao: ' + sat.orbit.altitudeKm + ' km</span>' +
                '<span class="sat-toggle-line">Quỹ đạo: LEO | Pha bay: ' + sat.orbit.phaseDeg.toFixed(0) + '&deg;</span>' +
                '</span>' +
                '</span>';
    ...
    setInterval(function() {
        satObjects.forEach(function(sat) {
            ...
            sat.footprintGlow.setLatLngs(buildFootprintSegments(pos, sat.orbit));
